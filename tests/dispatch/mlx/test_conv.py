...
def rng():
    return np.random.default_rng(sum(map(ord, "mlx conv")))
...
def test_the_conv_op_adds_its_bias(rng):
    ...
    compare_mlx_and_py([X, W, b], out, [X_np, W_np, b_np], assert_fn=assert_close)
...
def test_a_conv1d_layer_matches_end_to_end(rng):
    ...
    compare_mlx_and_py([X], layer(X), [X_np], assert_fn=assert_close)
...
def test_the_gradient_dispatch_matches_the_graph(spatial, rng):
    ...
    for wrt in (layer.W, layer.b, X):
        ...
        assert_close(
            np.asarray(pytensor.function([X], gradient, mode="MLX")(X_np)), np.asarray(reference)
        )
...
def test_a_lone_gradient_dispatches_and_matches_the_graph(wrt, rng):
    ...
    fn = pytensor.function([X], gradient, mode="MLX")
    grad_op = next(
